...
func (s *Store) loadFromDB(handle *sql.DB) (bool, error) {
	...
	if oauthDoc, ok, loadErr := loadSetting(handle, "oauth"); loadErr != nil {
		return false, loadErr
	} else if ok {
		_ = json.Unmarshal([]byte(oauthDoc), &s.oauthSettings)
	}
	if encDoc, ok, loadErr := loadSetting(handle, "enc_key"); loadErr != nil {
		return false, loadErr
	} else if ok {
		s.encryptionKeyRaw = encDoc
	}
...
func (s *Store) saveLocked() error {
	...
	oauthJSON, err := json.Marshal(s.oauthSettings)
	if err != nil {
		return fmt.Errorf("marshal oauth settings: %w", err)
	}
	if err := upsertSetting(tx, "oauth", string(oauthJSON)); err != nil {
		return err
	}
	if err := upsertSetting(tx, "enc_key", s.encryptionKeyRaw); err != nil {
		return err
	}
