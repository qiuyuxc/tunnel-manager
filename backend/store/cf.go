...
// ---------------------------------------------------------------------------
// OAuth client settings (admin panel; environment variables remain fallback)

// GetOAuthSettings returns the OAuth client configured in the admin panel.
func (s *Store) GetOAuthSettings() models.OAuthSettings {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.oauthSettings
}

// SetOAuthSettings persists the OAuth client configuration.
func (s *Store) SetOAuthSettings(settings models.OAuthSettings) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	previous := s.oauthSettings
	s.oauthSettings = settings
	if err := s.saveLocked(); err != nil {
		s.oauthSettings = previous
		return err
	}
	return nil
}
