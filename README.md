...
- `/DNS详情 [域名或ZoneID] [记录名称]`：查看单条记录的完整信息（含记录 ID、TTL、代理状态、MX 优先级）
- `/DNS添加`、`/DNS修改`：管理 A、AAAA、CNAME、TXT、MX，支持 TTL、代理状态和 MX 优先级
- `/DNS删除 [域名或ZoneID] [RecordID]`：生成五分钟有效的一次性确认码，必须再发送 `/确认删除 [确认码]` 才会删除
