...
没有初始密码：面板还没有账户时，打开面板会进入安装引导，管理员用户名和密码由你自己填写，不会写进日志。

如果你希望启动时就把账户建好（例如自动化部署），在首次启动前设置环境变量 `ADMIN_PASSWORD` 即可，它会成为管理员密码；之后再打开面板就是登录页。

### 安装引导没出现，或者我想换数据库？

安装引导只在面板还没有任何账户时出现。想重走一遍（换 SQLite / PostgreSQL，或改连接信息）：存储是引导自己选的，就删掉数据目录里的 `setup.json` 再重启；存储来自环境变量 `DATABASE_URL` / `STORE_PATH`，引导不会询问存储，改环境变量即可。

换数据库等于全新实例：新库里没有任何账户，首次打开面板会重新走安装引导，旧数据不会自动搬迁（见[升级、备份与恢复](/guide/upgrade-backup)）。
