...
| Pin the version | Replace the `latest` tag with a specific one: released versions accept semantic tags like `v2.4.0` or `2.4`, and any commit can be pinned with `sha-<short-hash>` |
