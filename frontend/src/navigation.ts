...
// One icon set, one menu list, shared by the desktop sidebar and the phone tab
// bar. Keeping them in a component meant the two drifted the moment either was
...
export const icons = {
  ...
  admin: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>',
...
export interface NavItem {
  path: string
  /** Sidebar wording; `tabLabel` is the shorter phone equivalent when it differs. */
  label: string
  tabLabel?: string
  icon: string
  group: NavGroup
...
export const NAV_ITEMS: NavItem[] = [
  ...
  { path: '/tunnels', label: '隧道管理', icon: icons.tunnels, group: 'workspace', perm: 'tunnels' },
...
export const TAB_PATHS: readonly string[] = ['/dashboard', '/monitors', '/dns']

/** Paths offered by the "+" button. */
export const CREATE_ACTIONS = [
  {
    path: '/tunnels',
    title: '新建隧道',
    desc: '创建 Cloudflare Tunnel 并接入主机',
    icon: icons.tunnels,
  },
  {
    path: '/domain',
...
/** `/dashboard` is exact so it does not swallow every other route. */
